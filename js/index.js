...
const CreateSpec = (type, interval_y, maximum_y, label_place, rows) =>
{
	...
	if (type === 'column' || type === 'stackedColumn')
	{
		spec_copy.axisY.interval = interval_y;
		spec_copy.axisY.maximum = maximum_y;
	}
...
const CreateChart = (id, titles, height, spec) =>
{
	...
	if (spec.data[0].type === 'column' || spec.data[0].type === 'stackedColumn')
	...
	if (spec.data[0].type === 'column' || spec.data[0].type === 'stackedColumn')
	{
		chart.set('dataPointWidth', Math.ceil(chart.axisX[0].bounds.width / chart.data[0].dataPoints.length) - 20);
		window.addEventListener('resize', _ => chart.set('dataPointWidth', Math.ceil(chart.axisX[0].bounds.width / chart.data[0].dataPoints.length) - 20));
	}
};
...
const PrepareToPrint = isExternal =>
{
	...
	style_print.onload = () =>
	{
		...
		{
			chart.render();
			if (chart.options.data[0].type === 'column' || chart.options.data[0].type === 'stackedColumn')
				chart.set('dataPointWidth', Math.ceil(chart.axisX[0].bounds.width / chart.data[0].dataPoints.length) - 20);
		}
...
const freqs_length = freqs_values.length;

const SumFreqsOfDict = input_dict =>
{
	const keys = new Set(freqs_values.flatMap(freq => Object.keys(input_dict[freq])));
	return Object.fromEntries([...keys].map(key => [key, freqs_values.reduce((sum, freq) => sum + input_dict[freq][key], 0)]));
};
...
const AnalyzeBladderDiaryText = text =>
{
	...
	const credential_length = Number(lines_lw[0].split(': ')[1].replace('cm', '')) / 100;
	const credential_weight = Number(lines_lw[1].split(': ')[1].replace('kg', ''));
	...
	for (let y = 0; y < lines_all.length - 1; ++y)
	{
		...
		for (const line_day of lines_day)
		{
			...
			const io = data_day[0], time = data_day[1], freq = data_day[2], volume = Number(data_day[3].replace('ml', '')), kind = data_day[4];
		...
		const bladder_volume_total_ceil = Math.ceil(Math.max(inputs_volume_total, outputs_volume_total) / 500) * 500;
		const bladder_volume_total_spec = CreateSpec('column', 500, bladder_volume_total_ceil, 'inside', bladder_volume_total_data);
		CreateChart('bladder_volume_total_div', [`Bladder Volume Total (ml) - ${yi}. Day`], bladder_volume_total_ceil / 250 * 25, bladder_volume_total_spec);
		...
		const inputs_volume_kind_spec = CreateSpec('doughnut', 80, null, 'outside', inputs_volume_kind_data);
		CreateChart('inputs_volume_kind_div', [`Inputs Kind (ml) - ${yi}. Day`, `Total: ${inputs_volume_total}ml`], 450, inputs_volume_kind_spec);
		...
		const inputs_volume_freq_spec = CreateSpec('pie', null, null, 'outside', inputs_volume_freq_data);
		CreateChart('inputs_volume_freq_div', [`Inputs Diurnal | Nocturnal Freq (ml) - ${yi}. Day`, `Total: ${inputs_volume_total}ml`], 450, inputs_volume_freq_spec);
		...
		const outputs_volume_freq_spec = CreateSpec('pie', null, null, 'outside', outputs_volume_freq_data);
		CreateChart('outputs_volume_freq_div', [`Outputs Diurnal | Nocturnal Freq (ml) - ${yi}. Day`, `Total: ${outputs_volume_total}ml`], 450, outputs_volume_freq_spec);
	}
	...
	freqs_values.forEach((freq, i) =>
	{
		...
		for (let j = 0; j < Object.keys(inputs_volume_range_dict[freq]).length; ++j)
		{
			inputs_volume_range_data[i].dataPoints.push(
			{
				...
				indexLabel: inputs_volume_range_dict[freq][j] ? `${inputs_volume_range_dict[freq][j]}pcs` : ''
			});
		}
	});

	const inputs_volume_range_ceil = Math.ceil(Math.max(...Object.values(SumFreqsOfDict(inputs_volume_range_dict))) / 2) * 2;
	const inputs_volume_range_spec = CreateSpec('stackedColumn', 2, inputs_volume_range_ceil, 'inside', inputs_volume_range_data);
	...
	], inputs_volume_range_ceil * 33, inputs_volume_range_spec);
	...
	freqs_values.forEach((freq, i) =>
	{
		...
		for (let j = 0; j < Object.keys(inputs_time_diff_range_dict[freq]).length; ++j)
		{
			inputs_time_diff_range_data[i].dataPoints.push(
			{
				...
				indexLabel: inputs_time_diff_range_dict[freq][j] ? `${inputs_time_diff_range_dict[freq][j]}pcs` : ''
			});
		}
	});

	const inputs_time_diff_range_ceil = Math.ceil(Math.max(...Object.values(SumFreqsOfDict(inputs_time_diff_range_dict))) / 2) * 2;
	const inputs_time_diff_range_spec = CreateSpec('stackedColumn', 2, inputs_time_diff_range_ceil, 'inside', inputs_time_diff_range_data);
	...
	], inputs_time_diff_range_ceil * 33, inputs_time_diff_range_spec);
	...
	freqs_values.forEach((freq, i) =>
	{
		...
		for (let j = 0; j < Object.keys(outputs_volume_range_dict[freq]).length; ++j)
		{
			outputs_volume_range_data[i].dataPoints.push(
			{
				...
				indexLabel: outputs_volume_range_dict[freq][j] ? `${outputs_volume_range_dict[freq][j]}pcs` : ''
			});
		}
	});

	const outputs_volume_range_ceil = Math.ceil(Math.max(...Object.values(SumFreqsOfDict(outputs_volume_range_dict))) / 2) * 2;
	const outputs_volume_range_spec = CreateSpec('stackedColumn', 2, outputs_volume_range_ceil, 'inside', outputs_volume_range_data);
	...
	], outputs_volume_range_ceil * 33, outputs_volume_range_spec);
	...
	freqs_values.forEach((freq, i) =>
	{
		...
		for (let j = 0; j < Object.keys(outputs_time_diff_range_dict[freq]).length; ++j)
		{
			outputs_time_diff_range_data[i].dataPoints.push(
			{
				...
				indexLabel: outputs_time_diff_range_dict[freq][j] ? `${outputs_time_diff_range_dict[freq][j]}pcs` : ''
			});
		}
	});

	const outputs_time_diff_range_ceil = Math.ceil(Math.max(...Object.values(SumFreqsOfDict(outputs_time_diff_range_dict))) / 2) * 2;
	const outputs_time_diff_range_spec = CreateSpec('stackedColumn', 2, outputs_time_diff_range_ceil, 'inside', outputs_time_diff_range_data);
	...
	], outputs_time_diff_range_ceil * 33, outputs_time_diff_range_spec);

	document.body.insertAdjacentHTML('beforeend', `<div class='br20'></div>`);
};